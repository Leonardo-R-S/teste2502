...
class Motorista {
	...
	function bindParams($conn, $sql, $usuario, $insert) {
		...
		$criacao_usuario = 'WS-'.$usuario['descricao'];
	...
	function insertQuery() {
		return "insert into kronaone.motoristas set 
            nome = :nome, 
            cpf = :cpf, 
            rg = :rg, 
    ...
    public function cadastrarMotorista($usuario){
        ...
        if(count($motoristaBanco) > 0){
			/*if ($usuario['ws_atualiza_motorista']) { // && $id == $dados['id']) {
				$sql = updateQuery();
				$smt2 = bindParams($sql, $usuario, false);
				$smt2->execute();
			}*/
            ...
            $sql = insertQuery();
		    $smt2 = bindParams($conn, $sql, $usuario, true);
