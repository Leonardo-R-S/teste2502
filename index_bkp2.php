...
$app->post('/CadastroMotorista', function(\Slim\Http\Request $req) {
    ...
    if(count($erros) > 0){
		...
		/*try{
			$motoristaModel = new \Model\Motorista($conn, null);
			$idMotorista = $motoristaModel->adicionarMotorista($dados["empresa_id"], $dados, $usuario);
		}  catch (Exception $e){
            $mensagem = "Não foi possível adicionar motorista. ".$e->getMessage();
		}*/	
	
		
        if($tipoRequisicao == "application/json"){
            $json['Resposta']['mensagem'] = $mensagem;
            $json['Resposta']['idMotorista'] = $idMotorista;
            return json_encode($json);
        }else if($tipoRequisicao == "application/xml"){
            $xml = new SimpleXMLElement("<resposta/>");
            $xml->addChild("mensagem", $mensagem);
...
$app->post('/CadastroChecklist', function(\Slim\Http\Request $req){
    ...
    //Verifica de usuario e senha são validos-------------------------------------------------------------
    $conn = \Apoio\Conexoes::conectar170();
    $sql = "select ope.id, ope.descricao, ope.checklist_simultaneos from kronaone.operacoes ope where ope.descricao = :usuario and ope.senha = :senha ";
    $ste = $conn->prepare($sql);
    $usu = $dadosHeader['HTTP_USUARIO'][0];
    $sen = sha1($dadosHeader['HTTP_SENHA'][0]);
    $ste->bindParam(':usuario', $usu);
    ...
    if(count($checklistBanco) > $usuario[0]["checklist_simultaneos"]){
...
$app->post('/ConsultaChecklist', function(\Slim\Http\Request $req){
    ...
    if(strlen($dados["placa"]) < 6 || strlen($dados["placa"]) > 7){
    ...
    if(count($erros) > 0){
        ...
        WHERE
        vei.placa = '{$placa}' order by ckl.id desc limit 1)
        union
        (SELECT
        ckl.id,
        ckl.criacao_datahora,
        ...
        join operacoes ope on ope.id = ckl.operacao_id
        WHERE
        reb1.placa = '{$placa}' order by ckl.id desc limit 1)
        union
        (SELECT
        ckl.id,
        ckl.criacao_datahora,
        ...
        join operacoes ope on ope.id = ckl.operacao_id
        WHERE
        reb2.placa = '{$placa}' order by ckl.id desc limit 1)
        union
        (SELECT
        ckl.id,
        ckl.criacao_datahora,
        ...
        join operacoes ope on ope.id = ckl.operacao_id
        WHERE
        reb3.placa = '{$placa}' order by ckl.id desc limit 1)")->fetchAll();
        ...
        if(count($check) < 1){
            ...
            if($tipoRequisicao == "application/json"){
                $json['Resposta']['idChecklist'] = $check[0]["id"];
                $json['Resposta']['status'] = $check[0]["checklist_status"];
                $json['Resposta']['dataCriacao'] = $check[0]["criacao_datahora"];
                $json['Resposta']['placaVeiculo'] = $check[0]["principal"];
                $json['Resposta']['placaReboque1'] = ($check[0]["reboque1"] == null ? "" : $check[0]["reboque1"]);
                $json['Resposta']['placaReboque2'] = ($check[0]["reboque2"] == null ? "" : $check[0]["reboque2"]);
                $json['Resposta']['placaReboque3'] = ($check[0]["reboque3"] == null ? "" : $check[0]["reboque3"]);
                $json['Resposta']['dataVigencia'] = $check[0]["vigencia"];
                return json_encode($json);
            }else if($tipoRequisicao == "application/xml"){
                $xml = new \SimpleXMLElement("<?xml version=\"1.0\" encoding=\"UTF-8\"?><resposta/>");
                $xml->addChild("idChecklist", $check[0]["id"]);
                $xml->addChild("status", $check[0]["checklist_status"]);
                $xml->addChild("dataCriacao", $check[0]["criacao_datahora"]);
                $xml->addChild("placaVeiculo", $check[0]["principal"]);
                $xml->addChild("placaReboque1", ($check[0]["reboque1"] == null ? "" : $check[0]["reboque1"]));
                $xml->addChild("placaReboque2", ($check[0]["reboque2"] == null ? "" : $check[0]["reboque2"]));
                $xml->addChild("placaReboque3", ($check[0]["reboque3"] == null ? "" : $check[0]["reboque3"]));
                $xml->addChild("dataVigencia", $check[0]["vigencia"]);
                return $xml->asXML();
            }
        }
    }
